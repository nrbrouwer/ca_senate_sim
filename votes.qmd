...
bills$bill_number <- paste0("SB-", bills$bill_number)
...
dat <- dat %>%
  ...
  select(Date, Bill, any_of(s_names), Vote_correct, Result, Dem_percent, Rep_percent) 

# --- RENDER with gt ---
dat |>
  gt() |>
  tab_options(
    table.width = pct(90)     # 90% of container width
  ) %>%
  tab_header(
    title = md("**Votes**"),
...
# --- CONFIG: your published CSV link ---
votes_csv <- "https://docs.google.com/spreadsheets/d/e/2PACX-1vSWsxVKMyPrvGZW1VFD0_DdTsMmH-dzITniXvWusbbG34FPwj7uWsIDB6B_6Sb5AdK94SbZ75eL0vTT/pub?gid=1971997036&single=true&output=csv"   
...
# --- CONFIG: your published CSV link ---
votes_csv <- "https://docs.google.com/spreadsheets/d/e/2PACX-1vSWsxVKMyPrvGZW1VFD0_DdTsMmH-dzITniXvWusbbG34FPwj7uWsIDB6B_6Sb5AdK94SbZ75eL0vTT/pub?gid=578574660&single=true&output=csv"   

# --- READ ---
dat <- read.csv(votes_csv) %>%
  rename_with(~ gsub("\\.", " ", .x)) %>%
  mutate(Bill = as.character(Bill))
